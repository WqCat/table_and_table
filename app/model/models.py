...
from app.db.base import Base, metadata_obj
...
class InboxObject(Base, BaseObject):
    ...
    relates_to_inbox_object: Mapped[Optional["InboxObject"]] = relationship(
        ...
        remote_side=id,  # 会使用 InboxObject 表的 id 字段与当前模型的关联字段进行匹配
        uselist=False,
    )
...
class Follower(Base):
    ...
    inbox_object = relationship(InboxObject, uselist=False) # uselist = False一对一
